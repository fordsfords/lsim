...
ERR_F lsim_dev_fadder_get_out_terminal(lsim_t *lsim, lsim_dev_t *dev, const char *out_id, lsim_dev_out_terminal_t **out_terminal, int bit_offset) {
  (void)lsim;
  ERR_ASSRT(dev->type == LSIM_DEV_TYPE_FADDER, LSIM_ERR_INTERNAL);

  ERR_ASSRT(bit_offset == 0, LSIM_ERR_COMMAND);  /* Only one output. */
  if (strcmp(out_id, "q0") == 0) {
    *out_terminal = dev->fadder.q_terminal;
  }
  ...
  return ERR_OK;
}  /* lsim_dev_fadder_get_out_terminal */


ERR_F lsim_dev_fadder_get_in_terminal(lsim_t *lsim, lsim_dev_t *dev, const char *in_id, lsim_dev_in_terminal_t **in_terminal, int bit_offset) {
  (void)lsim;
  ERR_ASSRT(dev->type == LSIM_DEV_TYPE_FADDER, LSIM_ERR_INTERNAL);

  ERR_ASSRT(bit_offset == 0, LSIM_ERR_COMMAND);  /* Only one output. */
  if (strcmp(in_id, "S0") == 0) {
    *in_terminal = dev->fadder.S_terminal;
  }
  else if (strcmp(in_id, "R0") == 0) {
    *in_terminal = dev->fadder.R_terminal;
  }
  else if (strcmp(in_id, "d0") == 0) {
    *in_terminal = dev->fadder.d_terminal;
  }
  else if (strcmp(in_id, "c0") == 0) {
    *in_terminal = dev->fadder.c_terminal;
  }
  else ERR_THROW(LSIM_ERR_INTERNAL, "unrecognized in_id '%s'", in_id);

  return ERR_OK;
}  /* lsim_dev_fadder_get_in_terminal */


ERR_F lsim_dev_fadder_power(lsim_t *lsim, lsim_dev_t *dev) {
  (void)lsim;
  ERR_ASSRT(dev->type == LSIM_DEV_TYPE_FADDER, LSIM_ERR_INTERNAL);
  ...
  return ERR_OK;
}  /* lsim_dev_fadder_power */


ERR_F lsim_dev_fadder_run_logic(lsim_t *lsim, lsim_dev_t *dev) {
  (void)lsim;
  ERR_ASSRT(dev->type == LSIM_DEV_TYPE_FADDER, LSIM_ERR_INTERNAL);

  ERR_THROW(LSIM_ERR_INTERNAL, "run logic should not be called for fadder");

  return ERR_OK;
}  /* lsim_dev_fadder_run_logic */


ERR_F lsim_dev_fadder_propagate_outputs(lsim_t *lsim, lsim_dev_t *dev) {
  (void)lsim;
  ERR_ASSRT(dev->type == LSIM_DEV_TYPE_FADDER, LSIM_ERR_INTERNAL);

  ERR_THROW(LSIM_ERR_INTERNAL, "propagate outputs should not be called for fadder");

  return ERR_OK;
}  /* lsim_dev_fadder_propagate_outputs */


ERR_F lsim_dev_fadder_delete(lsim_t *lsim, lsim_dev_t *dev) {
  (void)lsim;
  ERR_ASSRT(dev->type == LSIM_DEV_TYPE_FADDER, LSIM_ERR_INTERNAL);
  ...
  return ERR_OK;
}  /* lsim_dev_fadder_delete */


/* The fadder is a bit complex. Refer to the schematic:
 * https://raw.githubusercontent.com/fordsfords/lsim/refs/heads/main/fadder.svg */
ERR_F lsim_dev_fadder_create(lsim_t *lsim, char *dev_name) {
  ...
  dev->fadder.q_terminal = nand_q_dev->nand.o_terminal;
  dev->fadder.Q_terminal = nand_Q_dev->nand.o_terminal;
  /* Save references to the "external" input terminals. */
  ERR(lsim_dev_in_chain_add(&dev->fadder.S_terminal, nand_a_dev->nand.i_terminals[0], NULL));
  ERR(lsim_dev_in_chain_add(&dev->fadder.S_terminal, nand_q_dev->nand.i_terminals[0], NULL));
  ERR(lsim_dev_in_chain_add(&dev->fadder.c_terminal, nand_b_dev->nand.i_terminals[1], NULL));
  ERR(lsim_dev_in_chain_add(&dev->fadder.c_terminal, nand_c_dev->nand.i_terminals[1], NULL));
  ERR(lsim_dev_in_chain_add(&dev->fadder.d_terminal, nand_d_dev->nand.i_terminals[1], NULL));
  ERR(lsim_dev_in_chain_add(&dev->fadder.R_terminal, nand_b_dev->nand.i_terminals[2], NULL));
  ERR(lsim_dev_in_chain_add(&dev->fadder.R_terminal, nand_d_dev->nand.i_terminals[2], NULL));
  ERR(lsim_dev_in_chain_add(&dev->fadder.R_terminal, nand_Q_dev->nand.i_terminals[2], NULL));

  /* Type-specific methods (inheritance). */
  dev->get_out_terminal = lsim_dev_fadder_get_out_terminal;
  dev->get_in_terminal = lsim_dev_fadder_get_in_terminal;
  dev->power = lsim_dev_fadder_power;
  dev->run_logic = lsim_dev_fadder_run_logic;
  dev->propagate_outputs = lsim_dev_fadder_propagate_outputs;
  dev->delete = lsim_dev_fadder_delete;

  /* Write the fadder dev. */
